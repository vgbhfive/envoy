...
* 过滤器应该被配置为 *envoy.filters.network.dubbo_proxy* 的名称。
...
每个 dubbo 代理过滤器的配置都有一个基于 *dubbo.<stat_prefix>.* 如下所示的统计信息：
...
.. csv-table::
  ...
  cx_destroy_local_with_active_rq, Counter, 活跃查询中本地连接销毁总数
  cx_destroy_remote_with_active_rq, Counter, 活跃查询中远程连接销毁总数
